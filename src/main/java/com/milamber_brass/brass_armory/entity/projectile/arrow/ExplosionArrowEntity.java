...
public class ExplosionArrowEntity extends AbstractSpecialArrowEntity {
    ...
    protected void onHitBlock(BlockHitResult hitResult) {
        if (!this.level().isClientSide) {
            ArmoryUtil.explode(this.level(), this, hitResult.getLocation(), 2.0F, this.isOnFire(), Level.ExplosionInteraction.TNT);
            this.discard();
        }
    }
    ...
    protected void onHitEntity(EntityHitResult entityHitResult) {
        if (!this.level().isClientSide) {
            ArmoryUtil.explode(this.level(), this, entityHitResult.getLocation(), 2.0F, this.isOnFire(), Level.ExplosionInteraction.TNT);
            this.discard();
        }
    }
