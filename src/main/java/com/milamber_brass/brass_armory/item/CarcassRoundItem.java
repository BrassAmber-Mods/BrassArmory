...
public class CarcassRoundItem extends Item {
    ...
    public void appendHoverText(ItemStack itemStack, @Nullable Level level, List<Component> components, TooltipFlag tooltipFlag) {
        if (!isADragonRound(itemStack)) {
            components.add((new TranslatableComponent(PotionUtils.getPotion(itemStack).getName(Items.LINGERING_POTION.getDescriptionId() + ".effect."))).withStyle(ChatFormatting.GRAY));
    ...
    public @NotNull Component getName(ItemStack stack) {
        if (isADragonRound(stack)) return new TranslatableComponent("item.brass_armory.dragon_round");
